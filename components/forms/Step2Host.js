...
export default function Step2Host({ onSubmit, loading, savedAt, resetSignal }) {
  ...
  // ---- vehicle options caches (shared across all rows)
  ...
  const trimsCache = useRef(new Map()); // key: `${year}|${make}|${model}` -> { trims, specByTrim }
  ...
  async function fetchTrims(year, make, model) {
    if (!year || !make || !model) return { trims: [], specByTrim: {} };
    ...
    trimsCache.current.set(k, payload);
    return payload;
  }
  ...
  // parent-change helpers (clear dependents)
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <LabeledInput label="Pricing comfort (optional)">
          <input
...
/** Child: one row; owns options + prefill from specs */
function VehicleRow({
  index: i,
  row,
  yearList,
  ...
  }, [row.year, row.make, fetchModels]);

  useEffect(() => {
    let alive = true;
    if (row.year && row.make && row.model) {
      fetchTrims(row.year, row.make, row.model).then((p) => {
        if (!alive) return;
        setTrims(p.trims || []);
        setSpecByTrim(p.specByTrim || {});
      });
    } else {
      setTrims([]);
      setSpecByTrim({});
  ...
  }, [row.year, row.make, row.model, fetchTrims]);
...
/** small labeled wrappers for consistency */
